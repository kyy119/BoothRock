...
<body>
	...
	<c:forEach items="${booth_detail}" var="detail">
		<c:forEach items="${booth_product}" var="pd">

			<div class="bodywrap">
				<div class="booth-detail-form">
					<div class="booth-title">
						<h3 class="fstv-title">${detail.fstv_title}</h3>
						<a href="booth_detail.jsp" style="display: inline-block"><h1>${detail.booth_name}</h1></a>
						<span class="booth-type"> | ${detail.booth_category}</span>
					</div>
					<div class="booth-menu"
						style="width: 1000px; height: 300px; margin-top: 20px;">
						<div class="booth-img">
							<img alt="부스 이미지" src="resources/img/떡볶이.jpg">
							<img alt="부스 이미지" src="${detail.booth_img}"> 영윤님 경로 설정 후 지정
						</div>
						<div class="booth-item">
							<ul>
								<li class="booth-item-menu"><i
									class="fa-solid fa-chevron-left"></i> MENU <i
									class="fa-solid fa-chevron-right"></i></li>
								<li>${pd.product}<span>${pd.price}</span></li>
								<li>${pd.product}<span>${pd.price}</span></li>
								<li>${pd.product}<span>${pd.price}</span></li>
								<li>${pd.product}<span>${pd.price}</span></li>
								<li>${pd.product}<span>${pd.price}</span></li>
								...
								<div class="booth-info">
									<ul>
										<li class="booth-intro"><i class="fa-solid fa-quote-left"></i>
											${detail.booth_introduction} <i
											class="fa-solid fa-quote-right"></i></li>
										<li><i class="fa-solid fa-location-dot"
											style="color: #0080ff;"></i> ${detail.booth_addr}</li>
										<li><i class="fa-solid fa-clock" style="color: #ff8040;"></i>
											${detail.booth_hours}</li>
										<li><i class="fa-solid fa-phone" style="color: #00aa00;"></i>
											${detail.booth_tel}</li>
									</ul>
									<a href="report.jsp" class="report"><i
										class="fa-solid fa-triangle-exclamation"></i> 부스정보 신고하기 </a> <a
										href="#"
										onclick="duplicateReportCheck(${detail.booth_no}, '${user_id}')">
